...
"""Exact QAOA cost expectation from Qiskit Aer GPU statevectors.

The quantum state is produced exclusively by
``qiskit_aer.AerSimulator(method="statevector", device="GPU")`` through
:mod:`qaoa.aer_gpu`. This module materializes the Ising cost Hamiltonian as a
...
class QAOAExpectationError(RuntimeError):
    """Base error for exact QAOA expectation evaluation."""


class QiskitExpectationUnavailableError(QAOAExpectationError):
    """Raised when the required Qiskit quantum-info API is unavailable."""


class QiskitExpectationResultError(QAOAExpectationError):
...
def _finite_float(value: float, *, name: str) -> float:
    ...
    if not math.isfinite(normalized):
        raise QAOAExpectationError(f"{name} must be finite.")
    return normalized
...
def _load_qiskit_quantum_info() -> tuple[Any, Any, str]:
    ...
    try:
        ...
        raise QiskitExpectationUnavailableError(
...
class QAOAExpectationConfig:
    ...
    def fingerprint(self) -> str:
        ...
        digest.update(b"CSSF-QAOAExpectationConfig-v1\0")
        digest.update(
            json.dumps(
                {
...
@dataclass(frozen=True, slots=True)
class QiskitCostOperatorArtifact:
    ...
    def fingerprint(self) -> str:
        digest = hashlib.sha256()
        digest.update(b"CSSF-QiskitCostOperatorArtifact-v1\0")
        digest.update(
...
def build_qiskit_cost_operator(
    hamiltonian: IsingHamiltonian,
    ...
    return QiskitCostOperatorArtifact(
        operator=operator,
        terms=terms,
...
class QAOAExpectationResult:
    ...
    def fingerprint(self) -> str:
        digest = hashlib.sha256()
        digest.update(b"CSSF-QAOAExpectationResult-v1\0")
        digest.update(
            np.asarray(
                [
...
@dataclass(frozen=True, slots=True)
class QAOAGPUEvaluation:
    """Parameters, Aer GPU statevector, and exact cost statistics."""

    parameters: QAOAParameterValues
    statevector_result: AerStatevectorResult
    expectation_result: QAOAExpectationResult

    def __post_init__(self) -> None:
        if not isinstance(self.parameters, QAOAParameterValues):
            raise TypeError(
                "parameters must be QAOAParameterValues."
            )
        if not isinstance(
            ...
            raise TypeError(
                "statevector_result must be AerStatevectorResult."
            )
        if not isinstance(
            ...
            raise TypeError(
                "expectation_result must be QAOAExpectationResult."
            )
        if (
...
def evaluate_statevector_expectation(
    statevector_result: AerStatevectorResult,
    hamiltonian: IsingHamiltonian,
    *,
    config: QAOAExpectationConfig | None = None,
    operator_artifact: QiskitCostOperatorArtifact | None = None,
) -> QAOAExpectationResult:
    """Evaluate exact QAOA cost and audit it against the diagonal spectrum."""
    ...
    if not isinstance(run_config, QAOAExpectationConfig):
        raise TypeError(
            "config must be QAOAExpectationConfig or None."
        )
    ...
    artifact = (
        build_qiskit_cost_operator(hamiltonian)
        if operator_artifact is None
        else operator_artifact
    )
    if not isinstance(artifact, QiskitCostOperatorArtifact):
        raise TypeError(
    ...
    try:
        ...
        raise QiskitExpectationResultError(
...
def evaluate_qaoa_parameters_gpu(
    artifact: QAOACircuitArtifact,
    parameters: QAOAParameterValues,
    hamiltonian: IsingHamiltonian,
    *,
    aer_config: AerGPUConfig | None = None,
    expectation_config: QAOAExpectationConfig | None = None,
    operator_artifact: QiskitCostOperatorArtifact | None = None,
) -> QAOAGPUEvaluation:
    """Run the parameterized circuit on Aer GPU and evaluate exact cost."""

    if not isinstance(artifact, QAOACircuitArtifact):
        raise TypeError(
            "artifact must be QAOACircuitArtifact."
        )
    if not isinstance(parameters, QAOAParameterValues):
        raise TypeError(
            "parameters must be QAOAParameterValues."
        )
    ...
    statevector_result = run_qaoa_statevector_gpu(
        artifact,
        parameters,
        config=aer_config,
    )
    expectation_result = evaluate_statevector_expectation(
        statevector_result,
        hamiltonian,
        config=expectation_config,
        operator_artifact=operator_artifact,
    )
    return QAOAGPUEvaluation(
...
__all__ = [
    ...
    "QiskitExpectationUnavailableError",
    "QiskitExpectationResultError",
    "QAOAExpectationConfig",
    "QiskitCostOperatorArtifact",
    "QAOAExpectationResult",
    "QAOAGPUEvaluation",
