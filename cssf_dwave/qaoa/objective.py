...
"""Production objective callable for exact QAOA on Qiskit Aer GPU.

The objective delegates every quantum evaluation to
:func:`qaoa.expectation.evaluate_qaoa_parameters_gpu`, which in turn uses only
...
class QAOAObjectiveError(RuntimeError):
...
def _finite_float(value: float, *, name: str) -> float:
    ...
    if not math.isfinite(normalized):
        raise QAOAObjectiveError(f"{name} must be finite.")
    return normalized
...
class QAOAObjectiveConfig:
    ...
    def fingerprint(self) -> str:
        digest = hashlib.sha256()
        digest.update(b"CSSF-QAOAObjectiveConfig-v1\0")
        digest.update(
            json.dumps(
                {
...
class QAOAObjectiveRecord:
    ...
    def fingerprint(self) -> str:
        digest = hashlib.sha256()
        digest.update(b"CSSF-QAOAObjectiveRecord-v1\0")
...
class QAOAObjectiveSnapshot:
    ...
    records: tuple[QAOAObjectiveRecord, ...]
...
class QAOAGPUObjective:
    ...
    def __init__(
        ...
        *,
        ...
        config: QAOAObjectiveConfig | None = None,
        ...
        if (
            ...
            raise QAOAObjectiveError(
                "Exact QAOA objective exceeds the project-wide "
        ...
        self.aer_config = (
            AerGPUConfig() if aer_config is None else aer_config
        )
        ...
        if not isinstance(self.aer_config, AerGPUConfig):
            raise TypeError(
                "aer_config must be AerGPUConfig or None."
            )
        if not isinstance(
            self.expectation_config,
            QAOAExpectationConfig,
            ...
            raise TypeError(
                "expectation_config must be QAOAExpectationConfig "
                "or None."
            )
        if not isinstance(self.config, QAOAObjectiveConfig):
            raise TypeError(
                "config must be QAOAObjectiveConfig or None."
            )
        ...
        if not isinstance(
            ...
            raise TypeError(
                ...
                "QiskitCostOperatorArtifact or None."
            )
        if (
        ...
        self._records: deque[QAOAObjectiveRecord] = deque(
        ...
        self._best_evaluation: QAOAGPUEvaluation | None = None

    @property
    def parameter_count(self) -> int:
        return self.artifact.plan.parameter_layout.n_parameters
    ...
    @property
    def best_evaluation(self) -> QAOAGPUEvaluation | None:
        with self._lock:
    ...
    ) -> tuple[QAOAParameterValues, NDArray[np.float64]]:
        try:
            if isinstance(values, QAOAParameterValues):
                parameters = values
                if (
                    parameters.repetitions
                    != self.artifact.plan.config.repetitions
                ):
                    raise QAOAObjectiveError(
    ...
    def evaluate(
        ...
        with self._lock:
            ...
            try:
                evaluation = evaluate_qaoa_parameters_gpu(
                    self.artifact,
                    parameters,
                    self.hamiltonian,
    ...
    def fingerprint(self) -> str:
        ...
        digest = hashlib.sha256()
        digest.update(b"CSSF-QAOAGPUObjective-v1\0")
    ...
    def snapshot(self) -> QAOAObjectiveSnapshot:
