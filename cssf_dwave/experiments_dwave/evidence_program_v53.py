...
from experiments_dwave.pegasus_control_backend_v53 import (
...
PROGRAM_REVISION = "CSSF-QA-DWAVE-EVIDENCE-v53-20260819"
...
def run_level1_csnn_t_evidence(
    ...
    *,
    ...
    canonical frozen GCV lambda selection with the v53 SVD realization of the same Tikhonov objective.  The comparator
    ...
    cssf = fit_csnn_t_surrogate(ds, case="case300_level1_full_csnnt", n_lambdas=100, lam_range=(-12, 4), project_root=root)
    ...
    return {
        ...
        "cssf_numerical_solver": cssf.metadata.get("numerical_realization"),
        "cssf_conditioning": cssf.metadata.get("conditioning"),
        "cssf_mse": cssf_mse,
...
def load_runtime_config(project_root: str | Path, mode: str, *, solver_id: str | None = None, live_qpu: bool = False):
    """Load frozen base config and apply only the versioned 2026 QPU-id view."""
    root = Path(project_root)
    ...
    if not solver_id:
        raise EvidenceProgramError("An explicit Pegasus System4/System6 solver_id is required")
    sid = validate_pegasus_solver_id_v53(str(solver_id))
    cfg = load_config([root / "config" / "base.yaml", root / "config" / "case300.yaml", root / "config" / "pegasus_qpu.yaml"])
    # Do not mutate the frozen Pydantic qpu object with a post-2026 exact name;
    # expose a versioned runtime view instead.
    qpu_view = qpu_config_v53(cfg.qpu, solver_id=sid, live_qpu=bool(live_qpu))
    return cfg, qpu_view
...
def build_response_evaluator(
    ...
    *,
    ...
    root = Path(project_root)
    normalized = str(mode).strip().lower()
    runtime_cfg = load_runtime_config(root, normalized, solver_id=solver_id, live_qpu=live_qpu)
    if normalized == "qpu":
        cfg, qpu_cfg = runtime_cfg
    else:
        cfg, qpu_cfg = runtime_cfg, None
    ...
    if normalized == "qpu":
        if pegasus_solver_family_v53(str(solver_id)) != calibration_family:
            raise EvidenceProgramError("QPU solver family/calibration mismatch")
