...
class FixedPegasusControlBackend:
    ...
    def build(
        ...
        *,
        ...
        if normalized=="qpu":
            sid=str(getattr(bundle,"solver_id","") or "")
            if not sid.startswith(("Advantage_system4.","Advantage_system6.")):
                raise PegasusControlError("QPU solver must be Advantage_system4.* or Advantage_system6.*")
            if not sid.startswith(calibration.family+"."):
    ...
    def embedding_fingerprint(self) -> str:
        ...
        return _hash_json({"embedding":canonical,"chain_strength":self.chain_strength},"CSSF-Pegasus-FixedEmbedding-v38")
